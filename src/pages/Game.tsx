...
import { missions as originalMissions } from "@/data/missions";
...
const Game = () => {
  ...
  // 🔀 สุ่มลำดับ mission เมื่อ component mount
  const [missions] = useState(() => shuffleArray(originalMissions));
  ...
  // ✅ shuffle options เฉพาะใน mission ปัจจุบัน
  const shuffledOptions = useMemo(() => shuffleArray(mission.options), [mission.options]);
  ...
  const handleMissionComplete = useCallback(
    ...
    [currentMission, navigate, score, missions.length]
  );
  ...
  }, [timeLeft, handleMissionComplete]);

  useEffect(() => setTimeLeft(QUESTION_TIME_LIMIT), [currentMission]);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-background via-primary/5 to-accent/10 py-8 px-4 relative overflow-hidden">
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="container mx-auto max-w-6xl relative z-10">
        <ProgressBar current={currentMission + 1} total={missions.length} />

        <div className="text-center mb-4">
          <span className={`text-lg font-semibold ${timeLeft <= 5 ? "text-red-500 animate-pulse" : "text-primary"}`}>
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center mt-8">
          <div className="inline-flex items-center gap-3 bg-card/80 backdrop-blur-sm px-6 py-3 rounded-full shadow-lg border border-primary/20">
            ...
            <span className="text-2xl font-bold bg-gradient-to-r from-primary to-accent bg-clip-text text-transparent">{score}</span>
